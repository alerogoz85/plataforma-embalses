...
desplegar_web() {
  ( cd "$raiz/frontend"
    ...
    # `vercel link` agrega un token temporal a .env.local; se retira para no dejarlo en disco.
    [[ -f .env.local ]] && sed -i.bak '/^# Created by Vercel CLI/,$d' .env.local && rm -f .env.local.bak
    vercel deploy --prod --yes )
}
